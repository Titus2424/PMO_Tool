...
const getPermissionsFromSystemUser = (record: SystemUser | undefined): CurrentUserPermissions => {
  ...
  return {
    createProjects: getBooleanField(record, ['createProjects', 'create_projects', 'canCreateProjects', 'can_create_projects']),
    assignTasks: getBooleanField(record, ['assignTasks', 'assign_tasks', 'canAssignTasks', 'can_assign_tasks']),
    uploadDocuments: getBooleanField(record, ['uploadDocuments', 'upload_documents', 'canUploadDocuments', 'can_upload_documents']),
    reviewApprovals: getBooleanField(record, ['reviewApprovals', 'review_approvals', 'canReviewApprovals', 'can_review_approvals']),
  };
};
...
export function CurrentUserRoleProvider({ children }: { children: ReactNode }) {
  ...
  const systemUserQuery = useSystemUserList();
  ...
  const value = useMemo<CurrentUserRoleContextValue>(() => ({
    ...
    fullName,
    systemUser,
  }), [currentUserQuery.isLoading, email, fullName, systemUser, systemUserQuery.isLoading]);
